...
export default function CompletePage({ children }) {
  ...
  React.useEffect(
    function completePageAlgorithm() {
      if (!stripe) {
        return;
      }
      ...
      stripe
        ...
        .then(function paymentIntentAlgorithm({ paymentIntent }) {
          if (!paymentIntent) {
            return;
          }
  ...
  return (
    <div>
      <div style={{ background: STATUS_CONTENT_MAP[status].iconColor }}>
