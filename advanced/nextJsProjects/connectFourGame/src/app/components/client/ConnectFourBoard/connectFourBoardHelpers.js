...
function convertString(str) {
  const objOfString = {
    one: {
      uppercasedFirstStr: "One",
      spelledOutStr: "first",
    },
    two: {
      uppercasedFirstStr: "Two",
      spelledOutStr: "second",
    },
  };
  return objOfString[str];
}

function updateGameScore({ convertString, gameFuncObj, str }) {
  // get strings from obj
  const stringsObj = convertString(str);
  const { uppercasedFirstStr, spelledOutStr } = stringsObj;
  // update player score in game func obj
  console.log("gameFuncObj before", gameFuncObj);
  gameFuncObj[`player${uppercasedFirstStr}Score`] =
    gameFuncObj[`player${uppercasedFirstStr}Score`] + 1;
  console.log("gameFuncObj after", gameFuncObj);
  // update score UI display
  const playerScoreSpanElement = document.getElementById(
    `player-score-selector-${spelledOutStr}`
  ).childNodes[2];
  console.log(playerScoreSpanElement, "playerScoreSpanElement");
...
const methodsForRowOne = {
  ...
  fourth: function ({
    ...
    console.log("isWinnerGoingLeft", isWinnerGoingLeft.gameResult);
    if (isWinnerGoingLeft.gameResult == "winner") {
      console.log("isWinnerGoingLeft", isWinnerGoingLeft.gameResult);
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerGoingLeft.winningPlayer,
      });
      return;
    }
    ...
    console.log("isWinnerGoingTopLeft", isWinnerGoingTopLeft.gameResult);
    if (isWinnerGoingTopLeft.gameResult == "winner") {
      console.log("isWinnerGoingTopLeft", isWinnerGoingTopLeft.gameResult);
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerGoingTopLeft.winningPlayer,
      });
      return;
    }
    // get chips going top right
  ...
  sixth: function ({
    ...
    console.log("isWinnerHorizontalChips", isWinnerHorizontalChips.gameResult);
    if (isWinnerHorizontalChips.gameResult == "winner") {
      console.log(
        "isWinnerHorizontalChips",
        isWinnerHorizontalChips.gameResult
      );
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerHorizontalChips.winningPlayer,
      });
      return;
    }
...
const methodsForRowFour = {
  first: function ({
    ...
    console.log("isWinnerGoingDownRight", isWinnerGoingDownRight.gameResult);
    if (isWinnerGoingDownRight.gameResult == "winner") {
      console.log("isWinnerGoingDownRight", isWinnerGoingDownRight.gameResult);
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerGoingDownRight.winningPlayer,
      });
      return;
    }
  ...
  second: function ({
    ...
    console.log("isWinnerGoingDown", isWinnerGoingDown.gameResult);
    if (isWinnerGoingDown.gameResult) {
      console.log("isWinnerGoingDown", isWinnerGoingDown.gameResult);
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerGoingDown.winningPlayer,
      });
      return;
    }
  ...
  fourth: function ({
    ...
    console.log("isWinnerGoingDownLeft", isWinnerGoingDownLeft.gameResult);
    if (isWinnerGoingDownLeft.gameResult == "winner") {
      console.log("isWinnerGoingDownLeft", isWinnerGoingDownLeft.gameResult);
      updateGameScore({
        convertString,
        gameFuncObj,
        str: isWinnerGoingDownLeft.winningPlayer,
      });
      return;
    }
    // get chips going up right
...
function rowCounter({
  ...
  return function innerFunc(event) {
    ...
    if (
      ...
      console.log(
        "objOfCounters after running for loop to assign value null to index 0-7 of each subarrays",
        "reset objOfCounters",
        objOfCounters
      );
...
function connectFourChecker(getValuesForCheckFunc, arrays, gameFuncObj) {
  ...
  if (Array.isArray(isConnectFour) && isConnectFour.length == 4) {
    ...
    if (valueOfStringOneOrTwo == "one") {
      ...
      /**
       * need to move code that update players score out of connectFourChecker func
       * we are running this func multiple times.
       * **/
      console.log("gameFuncObj after", gameFuncObj);
    }
    ...
    // return an obj with the value of valueOfStringOneOrTwo
    return {
      gameResult: "winner",
      winningPlayer: valueOfStringOneOrTwo,
    };
  }
